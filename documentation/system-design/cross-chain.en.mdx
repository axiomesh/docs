...
Axiom's cross-chain mechanism is applicable to all EVM-compatible blockchains. Axiom's cross-chain mechanism ensures not only the cross-chain transmission of messages but also guarantees the transactions' integrity when Axiom interacts with other chains or when other chains interact with Axiom for asset exchanges.

# Cross-Chain Relayer

The cross-chain relayer is a crucial component within Axiom's cross-chain system. Its primary function is to relay cross-chain messages from one chain to another. The cross-chain relayer consists of two parts, each with distinct roles. Only when both parts are committed can the cross-chain message be effectively executed on the target chain. This mechanism prevents the possibility of malicious behavior from a single point.

Axiom has its own officially maintained cross-chain relayers. Axiom recommends users or applications to use the official cross-chain relayers to ensure the security of asset cross-chain operations and the accuracy of messages.

# Cross-Chain Manager Contracts

The cross-chain management contract serves as the engine within Axiom's cross-chain system. When a user or an on-chain application initiates a blockchain cross-chain request, it must go through the cross-chain management contract.
...
Cross-chain message delivery is relatively straightforward. Users and applications only need to send the message they want to deliver to the cross-chain management contract. Axiom's cross-chain system will then relay the message to the corresponding contract on the destination chain.
...
When a user initiates a cross-chain asset transfer, the assets will enter a semi-locked state. In this state, the assets are recorded in Axiom's cross-chain contract but not fully locked. The next steps of the process require waiting for the acknowledgment receipt from the cross-chain message before proceeding.
