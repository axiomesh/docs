...
![AxiomLedger 账本架构](../../images/system-design/smt-cn.png)
...
Axiomesh支持SPV（Simplified Payment Verification）。轻量级客户端（如钱包）在不存储完整的区块链账本的前提下，可以通过SPV协议向Axiomesh全节点请求默克尔证明，并以此验证账户状态与合约状态的正确性。SPV主要步骤如下：
1. 客户端从可信数据源下载区块头列表
2. 客户端向全节点请求Merkle证明
...
下图展示了Axiomesh SPV的主要步骤:

![Axiomesh SPV主要步骤](../../images/system-design/spv-cn.png)
