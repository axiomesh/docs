...
Axiom uses an account model. The address is the unique identifier of an account, and a private key is required to generate an account. An account has many attributes, including token balances, voting power, Etc. An account can:
...
All activities on Axiom are based on accounts.

# Account Type

Axiom has two account types:

- General account - that is to say, externally-owned account (EOA), controlled by anyone with the private key
- Contract account - a smart contract deployed on the Axiom network and controlled by code. No one owns the private key of the contract accounts.
...
For example, if Alice wants to send AXM from her account to Bob's, she must create and sign a transaction and send it to the Axiom network for verification. With the public key, Alice can prove that she originally initiated the transaction request. Without the encryption mechanism, malicious attacker Cathy might be able to publicly broadcast a similar request, "send 5 AXM from Alice's account to Cathy's account", while no one can validate whether or not the transaction is exactly requested by Alice herself.

# External-Owned Account(EOA) Creation

Axiom’s key pair generation algorithm is exactly the same as that of Ethereum, which uses the elliptic curve digital signature algorithm ECDSA-secp256k1. The process of key pair generation is:
...
The Axiom address in Hex format generated is exactly the same as the Ethereum address by the same private key.
...
When the smart contract is deployed to the Axiom network, the corresponding contract account address is returned, which is calculated from the ID of the deployed contract transaction and the sender's account address. The format of the contract account address is the same as that of external accounts. For example,
