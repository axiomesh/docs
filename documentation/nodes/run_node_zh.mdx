...
# 获取节点二进制文件
注意`AxiomLedger`二进制中包含了一个隐藏起来的`secret`，以保证只有相同`secret`的节点才能互相连接，而`secret`是在编译时打入二进制的，要连接到主网/测试网只能下载github上官方发布的二进制。
## 从github下载
...
配置文件位于`$WORKSPACE/axiom.toml`，主要需要修改对应端口以及创世验证节点配置，节点间通过P2P的DHT组网，因此只需配置创世种子节点的P2P地址。如果像快速启动的话，可以配置未solo节点模式，具体配置参考[配置说明]()。
TODO: 链接到配置文件说明稳定
