...
Go to the [Releases page](https://github.com/axiomesh/axiom/releases) of the `AxiomLedger` code repository, download the latest version binary zip file for your operating system and CPU architecture, and unzip it in the deployment environment to get the AxiomLedger binary.
...
Use the following command to compile the binary. Note that `secret` can be omitted. `net` can specify mainnet/testnet configs, if not specified it defaults to 4-node local config.
```shell
...
Config files are located at `$WORKSPACE/axiom.toml`. You mainly need to modify ports and genesis validator configs. Nodes connect via P2P DHT, so you only need to configure genesis seed node P2P addresses. For quick start you can use solo node mode, see [Config Docs]() for details.
