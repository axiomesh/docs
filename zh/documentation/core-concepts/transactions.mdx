...
改变链状态的交易需要被广播到整个网络。任何节点都可以广播交易请求。验证节点收到交易后，会执行该交易并将其包含在一个区块中，然后将区块传播到整个网络。

只有当验证节点将交易打包到一个区块中并且该区块得到确认后，交易才最终确认。
...
- 常规交易（Regular Transactions）：从一个账户发送到另一个账户的交易。这种类型的交易涉及将 AXC 从一个地址转移到另一个地址，并可以包括一个可选的数据字段以提供额外信息。

- 系统执行交易（System Execution Transaction）：这些交易用于与 AxiomLedger 网络原生集成的系统合约进行交互。在这种情况下，交易的“to”地址是系统合约的地址，交易数据字段包含要在合约内执行的函数调用和参数。

- 合约部署交易（Contract Deployment Transactions）：这些交易没有“to”地址，它们包括用于部署智能合约的代码和初始化参数。在这种类型的交易中，交易数据字段包含合约的字节码和构造函数参数。
