...
使用 NVM 安装 Node.js `v20` 和 `v16`，之后我们可能需要使用不同的 Node 版本来部署不同的 `Uniswap` 部件。
...
账户私钥用于在链上部署合约，可以用环境变量的方式来安全的使用，使用 `export YOUR_PRIVATE_KEY=‘YOUR_PRIVATE_KEY’` 命令来写入环境变量。最后以 aries 测试网为例 Hardhat config 配置如下所示
...
需要注意的是，配置在 hardhat network 中的第一个账户将会成为部署账号，您需要保证该账号中有足够的令牌，如何从水管获取令牌用于测试可以参考快速开始。
...
运行相应的部署脚本之后我们能获得相关的 Uniswap@V2 合约地址，运行相关的 ERC20 代币部署脚本之后我们能获得相关的 Token 合约地址
...
由于目前 Uniswap 支持 eth 主网，以及各个测试网，修改 `src/constants.ts` 中的 `ChainId[TESTNET]` 为我们所需要的 `23411`
...
进入相应的包，然后根据如下格式进行手动修改，WETH 是 Uniswap@V2 的必需合约，因此您部署完之后需要修改相应的 `address` 和 `chainId` 以及 `logoURI` 为您的相应配置。如下以 `src/tokens/testnet.json` 为例，请将上文部署 token 合约之后生产的 `WETH_ADDR`、`AXMTOKEN1_ADDR` 等填入相应的位置，测试令牌的 `logoURI` 可以自定义，修改 `chainId` 为 `aries` 的 `23411`。
...
修改环境配置，这里我们以 AxiomLedger 的 aries 测试网络为例。将项目目录下的 `.env` 和 `.env.production` 文件中的相应的条目修改为如下值：
...
修改完成之后运行如下命令即可在本地启动 Interface 前端，注意使用 Node@16
