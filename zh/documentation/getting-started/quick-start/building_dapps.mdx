...
<Warning>上述合约中的 `INIT_CODE_PAIR_HASH` 是 UniswapV2Pair 合约的 creationCode 的哈希值，需要你手动修改如下代码块。但是如果你是从 Axiomesh 项目库中获取的源码，则该部分修改已预先完成了。</Warning>
...
目前 Uniswap 支持 ETH 主网以及众多测试网，您可以任意修改 `src/constants.ts` 中的 `ChainId[TESTNET]` 为你需要的`链ID`。此处以AxiomLedger 测试网`Aries`为例，可将`链ID`修改为 `23411`：
