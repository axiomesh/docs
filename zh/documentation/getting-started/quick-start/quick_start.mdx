...
## 使用钱包连接AxiomLedger
AxiomLedger 是兼容EVM的区块链，任何您熟悉的以太坊工具都可以在 AxiomLedger 上使用，例如 `Truffle`、`Remix` 和 `ether.js` 等。您只需要拥有一个基于以太坊的账户地址，就可以通过主流的钱包（例如 Metamask 等）或 SDK 连接到 AxiomLedger 的测试网。

如果您是第一次使用MetaMask，请点击[此处](https://metamask.io/download/)安装插件。如果您是第一次连接 AxiomLedger，您需要将RPC 地址、链 ID、代币符号等信息新增到网络列表。主网及测试网的链信息可在[资源](/zh/documentation/getting-started/resources/resources)页查看。

连接到 AxiomLedger 后，一般来说您的账户余额是 `0` AXM，此时您可以访问 AxiomLedger [测试网水龙头](https://faucet.aries.axiomesh.io)，在页面上填入您的账户地址来获取一些AXM。

***请注意：在24小时内，一个账户只能在水龙头获取一次AXM！***
...
复制以下代码，将其保存为后缀为.mjs的文件，根据您的测试账户替换转账方的私钥以及接收方的地址（即代码中的account_from和account_to），然后执行 `node YourFileName.mjs`（请确保已提前安装Node.js）。等待终端返回交易哈希即代表交易已成功，此时可查看Metamask中的余额是否更新。若网络有延迟，可稍后查询。可以在[AXMScan](https://scan.aries.axiomesh.io/)中粘贴返回的交易哈希查看交易详情。

<Warning>需注意：若您的电脑上启动了代理服务可能导致交易失败，可关闭代理后再次尝试。</Warning>
...
为了降低您的使用难度，AxiomLedger提供了一份简单的智能合约样本，您只需要修改其中的Token名称（样本以TAXM为例）和账户地址即可。当然，您也可以参考此合约开发更符合您需要的智能合约。
``` solidity
...
如果您已经对以太坊有一定的了解，可以通过Remix编译并部署此智能合约。请注意：
- 编译合约时请选择合适的Solidity版本(^0.4.24)
- 通过injected Provider - MetaMask的方式部署，请确认网络已经切换到AxiomLedger测试网并且账户地址符合您的选择
- 已选择能正确生成Token的合约(例如该例中的`TestToken`)
...
至此，您成功在AxiomLedger上部署了ERC20标准的智能合约，并铸造了属于您的Token！当然，您也可以通过SDK完成智能合约的部署，具体的操作可以参考[ether.js](https://github.com/ethers-io/ethers.js)。
...
## 体验AxiomLedger区块链浏览器
AxiomLedger提供了官方的[区块链浏览器](https://scan.aries.axiomesh.io)。您在与AxiomLedger交互的过程中，可以随时查看测试网的区块/交易信息。 您可以在[此处](/zh/documentation/getting-started/resources/resources#axmscan)了解更多关于AXMScan的使用方法。