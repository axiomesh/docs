...
## 使用钱包连接AxiomLedger
AxiomLedger 是兼容EVM的区块链，任何您熟悉的以太坊工具都可以在 AxiomLedger 上使用，例如 Truffle、Remix 和 ether.js 等。您只需要拥有一个基于以太坊的账户地址，就可以通过主流的钱包（例如 Metamask 等）或 SDK 连接到 AxiomLedger 的测试网，其中测试网的 RPC 地址为：`https://rpc1.aries.axiomesh.io`，链ID为：`23411`，代币符号为 `AXM`。
如果您是第一次使用 Metamask 连接 AxiomLedger，您需要将上面的 RPC 地址、链 ID、代币符号等信息新增到网络列表，连接到 AxiomLedger 后，一般来说您的账户余额是 `0` AXM，此时您可以访问 AxiomLedger 测试网水龙头，在页面上填入您的账户地址来获取`0.5`AXM，水龙头的地址为：`https://faucet.aries.axiomesh.io`，***请注意：在24小时内，一个账户只能在水龙头获取一次AXM！***
