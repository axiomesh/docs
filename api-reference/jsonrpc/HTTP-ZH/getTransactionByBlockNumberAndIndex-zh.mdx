...
<ParamField body="method" type="string" placeholder="eth_getTransactionByBlockNumberAndIndex">
  进行调用的远程方法名称
</ParamField>
...
<ParamField body="params" type="array">
  ...
  <Expandable title="params">
     ...
     <ParamField body="TRANSACTION INDEX" type="string">
        表示块中交易位置的整数的十六进制
     </ParamField>
...
<ResponseField name="TRANSACTION" type="object">
  ...
  <Expandable title="TRANSACTION object">
    ...
    <ResponseField name="gasPrice" type="string">
        发送者以axm为基础单位提供的gas价格
    </ResponseField>

    <ResponseField name="gasFeeCap" type="string">
        maxFeePerGas，最高费用，以axm为单位，发送者愿意支付高于基本费用的gas
    </ResponseField>

    <ResponseField name="gasTipCap" type="string">
        maxPriorityFeePerGas，最高总费用（基本费用+优先费用），以axm为单位，发送方愿意支付每份gas费用
    </ResponseField>
