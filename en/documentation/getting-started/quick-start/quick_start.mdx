...
If you are using MetaMask for the first time, please click [here](https://metamask.io/download/) to install the plugin. If you are connecting to AxiomLedger for the first time, you need to add RPC address, chain ID, token symbol, and other required information to the network list.
...
First, create a new account to receive the Token. Then, call the `transfer` method at the bottom of the page where the contract was deployed just now (you will need to provide the address of the new account and the transfer amount). Finally, confirm the transaction in the MetaMask pop-up box.
