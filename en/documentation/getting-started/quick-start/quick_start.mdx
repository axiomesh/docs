...
AxiomLedger is an EVM-compatible blockchain, and any Ethereum tools you are familiar with can be used on AxiomLedger, such as Truffle, Remix, ether.js. You only need to have an account address based on Ethereum, and you can connect to AxiomLedger's Testnet through popular wallets (such as Metamask ) or SDK. The RPC address of AxiomLedger’s Testnet is: `https://rpc1.aries.axiomesh.io`, and the chain ID is: `23411`, The token symbol is `AXM`.

If you are using Metamask to connect to the AxiomLedger for the first time, you need to add the above RPC address, chain ID, token symbol and other information to the network list. After connecting to the AxiomLedger, generally speaking, your account balance is `0` AXM. At this time, you can visit the AxiomLedger testnet faucet and fill in your account address on the page to obtain `0.5` AXM. The address of the faucet is: `https://faucet.aries.axiomesh.io/`. **_Please note: within 24 hours, an account can only get AXM once on the faucet!_**

## Generating Your First Transaction

If you need to learn more about AxiomLedger,such as want to get more information on the chain or initiate transactions on the chain, it is recommended that you use the official SDK ([axiom.js](https://github.com/axiomesh/axiom.js)) provided by Axiom, which is similar to [ether.js](https://github.com/ethers-io/ethers.js) in usage.
...
In order to reduce the difficulty of your use, AxiomLedger provides a simple smart contract sample, you only need to modify the Token name (example is TAXM) in it(the location to be modified is at lines 62-63 of the smart contract code below). Of course, you can also refer to this contract to develop a smart contract that better meets your needs.
...
If you already have a certain understanding of Ethereum, you can compile and deploy this smart contract on Remix. **_Please note: 1. Please select the appropriate Solidity version when compiling the contract; 2. Deploy contract through injected Provider - MetaMask, please confirm that the network has been switched to the AxiomLedger’s network and the account address matches your choice._**
...
contracts through the SDK. For specific operations, please refer to ether.js.
