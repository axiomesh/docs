...
Adjust the environment configuration. Taking `AxiomLedger` `Taurus` testnet as an example, modify the corresponding entries in the `.env` and `.env.production`files to the following values:
