...
<Warning>The `INIT_CODE_PAIR_HASH` in the aforementioned contract is actually the hash value of the creationCode of the `UniswapV2Pair` contract. You need to manually modify the following code block. But if you have obtained the source code from the Axiomesh project repository, this part of the modification has already been completed.</Warning>
...
Currently, Uniswap supports the Ethereum mainnet as well as numerous testnets. You can freely modify the `ChainId[TESTNET]` in `src/constants.ts` to your desired `chain ID`. Here, we'll take the `AxiomLedger` testnet `Aries` as an example, and you can change the chain ID to `23411`.
