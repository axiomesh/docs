...
Homebrew is a package manager for MacOS that can help you easily install and manage software.
...
Install Node.js `v20` & `v16` using NVM. Later, we may need to use different Node versions to deploy different `Uniswap` components.
...
Since Uniswap currently supports the Ethereum mainnet and various testnets, modify the `ChainId[TESTNET]` in `src/constants.ts` to what we need:
...
Deploy the `WETH` contract on the testnet. Earlier, you've already deployed the relevant Token contract on the chain. Insert the address into the `WETH_ADDR` position in the following code:
...
Enter the corresponding package and manually modify it according to the following format. Since WETH is a required contract for Uniswap@V2, you need to modify the `address` and `chainId` to your configuration after deployment. Using `src/tokens/testnet.json` as an example, please fill in the corresponding locations with the `WETH_ADDR`, `AXMTOKEN1_ADDR`, etc., produced after deploying the token contract as mentioned above. The `logoURI` of the test token can be customized. Modify the `chainId` to aries's `23411`.
...
#### Modify Dependencies and Configuration
...
Now you can experience Uniswap@V2 built on AxiomLedger's aries testnet. Give liquidity addition a try!
