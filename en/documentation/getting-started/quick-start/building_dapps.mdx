...
The Hardhat config configuration is shown below, using the aries testnet as an example:
...
Adjust the environment configuration, using AxiomLedger's aries test network as an example:
...
After making the necessary modifications, execute the following commands to start the Interface frontend locally:
